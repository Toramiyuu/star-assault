...
export class PreloadScene extends Scene {
  ...
  preload() {
    ...
    // Drop icons — load AI-generated PNGs if they exist (generated by scripts/generate-drop-icons.js)
    // Falls back to generateTexture() in create() for any that fail to load
    for (const type of ['heart', 'shield', 'bomb', 'magnet', 'boost', 'elite_shard']) {
      this.load.image(`drop_${type}`, `assets/drops/${type}.png`);
    }
  ...
  _generateDropTextures() {
    ...
    for (const [type, cfg] of Object.entries(types)) {
      // If PNG loaded successfully (width > 4 = not a Phaser placeholder), skip generateTexture
      const existing = this.textures.get(`drop_${type}`);
      if (existing && existing.source[0] && existing.source[0].width > 4) continue;
